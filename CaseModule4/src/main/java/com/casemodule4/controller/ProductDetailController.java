...
public class ProductDetailController {
    ...
    public String upImg(@RequestParam MultipartFile fileImg) {
        ...
        try {
            FileCopyUtils.copy(fileImg.getBytes(), new File("C:\\Users\\Tran Thi Thu Hang\\Desktop\\FE_team5\\Team5_FrontEnd\\MarkUps-dailyShop\\dailyShop\\img\\" + nameImg));
            return "/FE_team5/Team5_FrontEnd/MarkUps-dailyShop/dailyShop/img/" + nameImg;
        } catch (IOException e) {
            e.printStackTrace();
        }
        return null;
    ...
    public Product showEditProduct(@PathVariable long idProduct){
        return productService.findProductById(idProduct);
    }
    @GetMapping("/size/{idSize}")
    public Size showEditSize(@PathVariable long idSize){
        return sizeService.findSizeById(idSize);
    }
    @GetMapping("/productDetail/{idProductDetail}")
    public ProductDetail showEditProductDetail(@PathVariable long idProductDetail){
    ...
    public Product deleteProduct(@PathVariable long idProduct) {
        return productService.deleteProductById(idProduct);
    }

    @DeleteMapping("/size/{idSize}")
    public Size deleteSize(@PathVariable long idSize) {
        return sizeService.deleteSize(idSize);
    }
    @DeleteMapping("/productDetail/{idProductDetail}")
    public ProductDetail deleteProductDetail(@PathVariable long idProductDetail) {
